<template>
  <div class="main user-layout-register">
    <h3><span>教师注册</span></h3>
    <a-form ref="formRegister" :autoFormCreate="(form)=>{this.form = form}" id="formRegister">
      <a-form-item
        fieldDecoratorId="username"
        :fieldDecoratorOptions="{rules: [{ required: true, message: '姓名不能为空'}], validateTrigger: ['change', 'blur'], validateFirst: true}">
        <a-input size="large" type="text" autocomplete="false" placeholder="请输入姓名"></a-input>
      </a-form-item>
      <a-form-item
        ...
        <a-input size="large" placeholder="手机号(作为登录账户)">
      ...
      <a-form-item
        fieldDecoratorId="collegeId"
        :fieldDecoratorOptions="{rules: [{ required: true, message: '学院不能为空'}], validateTrigger: ['change', 'blur'], validateFirst: true}">
        <a-select
          size="large"
          show-search
          placeholder="请输入学院名称"
  ...
  export default {
    ...
    data() {
      return {
        ...
        collegeList: [],
        url: {
          getCollegeNames: '/sys/queryCollegeNames'
        },
        collegeId: ''
      }
    },
    ...
    methods: {
      ...
      initCollege() {
        getAction(this.url.getCollegeNames, {}).then(res => {
          if (res.success) {
      ...
      handleSubmit() {
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            postAction('/sys/registerTeacher', register).then((res) => {
              if (!res.success) {
