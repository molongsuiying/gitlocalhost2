      ...
      <a-table
        ...
        <template slot="prizeType" slot-scope="text,record">
          ...
          <template v-else-if="text == 2">
            <font>立项人:{{ record.buildProjectCharge }}</font><br />
            <font>结项人:{{ record.overProjectCharge }}</font>
          </template>
          <template v-else>
...
<script>
  ...
  import Vue from 'vue'
  import HonourCertificateModal from './HonourCertificateModal'
...
import store from '../../../store'
  export default {
    ...
    methods: {
      ...
      handleDetail(record) {
        record.honourClass = 2
        record.honourClass_dictText = '证书类'
