<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="10">
            <a-form-item label="创建时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-range-picker style="width: 210px" v-model="queryParam.createTimeRange" format="YYYY-MM-DD"
    ...
    <!-- table区域-begin -->
    <a-table :scroll="scroll" bordered ref="table" size="middle" rowKey="id" :dataSource="dataSource" :pagination="ipagination"
      ...
      <a-table-column title="状态" dataIndex="status" :width="120" align="center" key="s" :sorter="(a,b)=>a.status - b.status">
        <template slot-scope="t,r,i">
          <span :style="{color:getStatus(t).color}"> {{getStatus(t).text}} </span>
        </template>
      </a-table-column>
      <a-table-column title="结果" dataIndex="result" :width="120" align="center" key="result" :sorter="(a,b)=>a.result - b.result">
        <template slot-scope="t,r,i">
      ...
      <a-table-column title="提交申请时间" dataIndex="applyTime" :width="160" align="center">
        <template slot-scope="t,r,i">
          <span> {{t}} </span>
        </template>
      </a-table-column>
      <a-table-column title="操作" dataIndex="" align="center">
        <template slot-scope="t,r,i">
          <template v-if="r.status == 0">
            <a href="javascript:void(0);" style="color: #00A0E9" @click="apply(r,0)">提交申请</a>
            <a-divider type="vertical" />
            <a href="javascript:void(0);" @click="detail(r)" style="color:#999;">表单数据</a>
            <a-divider type="vertical" />
            <a-popconfirm title="确定删除重选吗?" @confirm="() => remove(r)">
              <a href="javascript:void(0);" style="color: red">删除重选</a>
            </a-popconfirm>

          </template>
          <template v-else-if="r.status == 1">
            <template v-if="r.cancelFlag ==  1">
              <a href="javascript:void(0);" @click="cancel(r)" style="color:#8000ff;">撤回工单</a>
              <a-divider type="vertical" />
            </template>
          ...
          <template v-else-if="(r.status == 2 && r.result == 3) || r.status == 3">
            <a-popconfirm title="确定提交申请吗?" @confirm="() => apply(r,1)">
            ...
            <a href="javascript:void(0);" @click="detail(r)" style="color:#999;">表单数据</a>
            <a-divider type="vertical" />
            <a href="javascript:void(0);" @click="history(r)" style="color:blue;">流程历史</a>
          </template>
          <template v-else>
            <a href="javascript:void(0);" @click="detail(r)" style="color:#999;">表单数据</a>
            <a-divider type="vertical" />
    ...
    <a-modal title="提交申请" v-model="modalVisible" :mask-closable="false" :width="500" :footer="null">
      <div v-if="modalVisible">
        ...
        <a-form-item label="选择审批人" v-show="showCustomizedDepart">
          <!-- <j-select-depart v-model="form.departIds" :subDepartId="subDepartId" :multi="false"></j-select-depart> -->
        ...
        <a-form-item label="优先级">
          ...
          <component :is="LcDict" v-model="form.priority" style="width: 100%" placeholder="请选择" dictCode="ACTIVITI_PRIORITY"></component>
        </a-form-item>
    ...
    <a-modal title="审批历史" v-model="modalLsVisible" :mask-closable="false" :width="'80%'" :footer="null">
      <div v-if="modalLsVisible">
        <historicDetail :procInstId="procInstId" :procDefId="procDefId"></historicDetail>
      </div>
    </a-modal>
...
<script>
  ...
  import HonourDetailModal from '../school/modules/HonourDetailModal';
  export default {
    name: "applyList",
    ...
    methods: {
      ...
      handleDetail(r) {
        ...
        // isView = isView || false;
        // var abc = "@/views/school/modules/HonourDetailModal"
        // this.lcModa.disabled = isView;
        // // this.lcModa.title = '修改流程业务信息：' + r.title;
        // // if (isView) 
        // r.routeName = abc
        // console.log(r.routeName)
        // // this.lcModa.title = '查看流程业务信息：' + r.title;
        // this.lcModa.formComponent = this.getFormComponent(abc).component;
        // this.lcModa.processData = r;
        // this.lcModa.isNew = false;
        // this.lcModa.visible = true;
        r.honourClass = 1
        r.honourClass_dictText = '文件类'
        this.$refs.detail.title = r.honourClass_dictText + '详情'
        this.$refs.detail.detail(r)
      ...
      detail(v) {
        console.log(v.routeName)
        if(v.routeName=="@/views/school/modules/HonourFileForm"){
          this.handleDetail(v);
        }else{
        this.edit(v, true);
        }
