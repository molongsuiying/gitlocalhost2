    ...
    <a-card>
      ...
      <a-row>
        <a-table :scroll="scroll" bordered :loading="loading" rowKey="id" :dataSource="data" :pagination="ipagination"
          ...
          <a-table-column title="申请人" dataIndex="applyer" :width="100" align="center">
            <template slot-scope="t,r,i">
              <span> {{t}} </span>
            </template>
          </a-table-column>
          <a-table-column title="标识Key" dataIndex="key" :width="150" align="center">
            <template slot-scope="t,r,i">
              <span> {{t}} </span>
            </template>
          </a-table-column>
<!--          <a-table-column title="版本" dataIndex="version" :width="100">
            <template slot-scope="t,r,i">
              <span> v.{{t}} </span>
            </template>
          </a-table-column> -->
          <a-table-column title="当前环节" dataIndex="currTaskName" :width="150" align="center">
            <template slot-scope="t,r,i">
              <span> {{t}} </span>
            </template>
          </a-table-column>
          <a-table-column title="状态" dataIndex="isSuspended" align="center" :width="110" key="isSuspended" :sorter="(a,b)=>Boolean(a.isSuspended)?0:1 - Boolean(b.isSuspended)?0:1">
            <template slot-scope="t,r,i">
          ...
          <a-table-column title="操作" dataIndex="action">
            <template slot-scope="t,r,i">
              ...
              <a href="javascript:void(0);" style="color: blue;" @click="history(r)">流程历史</a>
              <a-divider type="vertical" />
              <a href="javascript:void(0);" style="color: #000;" @click="detail(r)">表单数据</a>
              <a-divider type="vertical" />
              <a href="javascript:void(0);" v-has="'operator:urge'" style="color: red;" @click="urgent(r,0)">催办</a>
              <a-divider type="vertical" />
              <a href="javascript:void(0);" v-has="'operator:supervise'" style="color: red;" @click="urgent(r,1)">督办</a>
    ...
    <a-modal :title="modalTitle" v-model="modalVisible" :mask-closable="false" :width="500" :styles="{top: '30px'}">
    ...
    <a-modal :title="modalTaskTitle" v-model="modalTaskVisible" :mask-closable="false" :width="500">
      ...
      <div v-if="modalTaskVisible">
        <a-form ref="form" :model="urge_form" :label-width="85">
          节点创建时间:<font style="margin-left: 10px;">{{urge_form.createTime}}</font>
          <a-form-item :label="modalTaskTitle+'留言'" prop="reason">
            <a-input type="textarea" v-model="urge_form.comment" :rows="4" />
          </a-form-item>

          <template v-if="urge_form.flag == 1">
            <a-form-item label="逾期日期" prop="dueTime" >
              <a-date-picker showTime :disabledDate="disabledDate" format="YYYY-MM-DD HH:mm:ss" v-model="urge_form.dueTime" />
            </a-form-item>
          </template>
         

          <a-form-item :label="modalTaskTitle+'审批人'" prop="assignees">
            <a-select v-model="urge_form.assignees" placeholder="请选择" allowClear mode="multiple" :loading="userLoading">
              <a-select-option v-for="(item, index) in assigneList" :key="index" :value="item.username">{{item.realname}}</a-select-option>
            </a-select>
          </a-form-item>

          <a-form-item label="消息通知">
            <a-checkbox v-model="urge_form.sendMessage">站内消息通知</a-checkbox>
            <a-checkbox v-model="urge_form.sendSms" disabled>短信通知</a-checkbox>
            <a-checkbox v-model="urge_form.sendEmail" disabled>邮件通知</a-checkbox>
          </a-form-item>
        </a-form>
      </div>
      <div slot="footer">
        <a-button type="text" @click="modalTaskVisible=false">取消</a-button>
        <a-button type="primary" :loading="urgentSubmitLoading" @click="urgentHandelSubmit">提交</a-button>
      </div>
    </a-modal>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          ...
          urgentTask: '/actTask/urgentTask',
          superviseTask:'/actTask/superviseTask',
        },
        ...
        urgentSubmitLoading: false, // 添加或编辑提交状态
        urge_form: {
          id: "",
          procInstId: "",
          comment: "",
    ...
    methods: {
      ...
      urgent(v,flag) {
        if(flag == 1){
          this.modalTaskTitle = "督办";
          this.urge_form.flag = 1;
        }else{
          this.modalTaskTitle = "催办"
          this.urge_form.flag = 0;
        }
        if (!v.procInstId) {
          this.$message.error("流程实例ID不存在");
          return;
        ...
        this.urge_form.assignees = [];
        this.assigneList = [];
        this.getAction(this.url.historicFlow + this.procInstId).then(res => {
          this.loading = false;
          if (res.success) {
            var data = res.result;
            if (data.length > 0) {
              var array = [];
              var assignees = data[0].users;
              this.urge_form.createTime = data[0].createTime;
              for (var i = 0; i < assignees.length; i++) {
                console.log(assignees[i]);
                array.push(assignees[i].username);
              }
              this.assigneList = data[0].users;
              this.urge_form.assignees = array;

              this.urge_form.procInstId = this.procInstId;
              this.urge_form.id = data[0].id;
            }
            console.log(array);
      ...
      urgentHandelSubmit() {
        console.log(this.urge_form);
        var formData = Object.assign({}, this.urge_form);

        var postUrl = this.url.urgentTask;
        if(formData.flag == 1){
          if (Date.parse(formData.dueTime) < Date.parse(formData.createTime)) {
            this.$message.error("督办日期要大于节点创建日期");
            return;
          }
          formData.dueTime = moment(formData.dueTime).format('YYYY-MM-DD HH:mm:ss');
          console.log(formData);
          postUrl = this.url.superviseTask;
        }
        
        
        formData.assignees = formData.assignees.join(",");
        if (formData.assignees == '' || formData.assignees == null) {
          this.$message.error("至少"+this.modalTaskTitle+"一人");
          return;
        }
        this.postFormAction(postUrl, formData).then(res => {
          this.urgentSubmitLoading = false;
          if (res.success) {
      ...
      disabledDate(current) {
        ...
        var time = this.urge_form.createTime;
