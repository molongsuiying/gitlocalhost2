<template>
  <a-card :bordered="false">
    ...
    <a-input-search style="margin-bottom: 10px;margin-right:10px;width: 200px" v-model="searchProcessKey"
                    placeholder="输入名称" @search="onSearchProcess" />
    <a-button @click="onSearchProcess(searchProcessKey)" type="primary">查询</a-button>
    <a-button @click="onSearchProcess('')" style='margin-left: 10px;'>重置</a-button>
    <!-- <a-button @click="handleToApplyList" type="primary" style="float: right;">前往我的申请列表</a-button> -->
      <a-empty description="无模型可供选择" v-if="activeKeyAll.length==0" />
      <div v-else>
  ...
  export default {
    ...
    data () {
      return {
        ...
        dictOptions:[],
        url: {
          getProComplainList: "/pro_complain/listData",
        },
        // 查询条件
        queryParam: {
          createTimeRange:[],
    ...
    methods: {
      ...
      getProcessList() {
        ...
        this.getAction(this.url.getProComplainList,{status:1,roles:true}).then(res => {
          this.activeKeyAll = [];
          if (res.success) {
