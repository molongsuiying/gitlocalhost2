          ...
          <a-list :grid="{ gutter: 10,column:4}" :dataSource="processDataMap[value]">
            <a-list-item slot="renderItem" slot-scope="item">
              <a-card>
                <div slot="title">
                  <a-row>
                    <template v-if="item.tableName == 'zd_honour_file'">
                  <a-col span="12" :title="item.leader">投诉人:<span style="margin-left: 10px;"></span>{{item.leader}}
                    </a-col>
                </template>
                    <template v-if="item.tableName == 'act_z_complain'">
  ...
  export default {
    ...
    methods: {
      ...
      detail(r) {
        ...
        if(r.tableName=="act_z_complain"){
            this.lcModa.title = '查看流程业务信息：' + r.name;
        }else{
        this.lcModa.title = '查看流程业务信息：' + r.workName;
       }
