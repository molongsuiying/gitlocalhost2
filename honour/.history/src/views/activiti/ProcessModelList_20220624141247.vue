<template>
  <a-card :bordered="false">
    ...
    <!-- table区域-begin -->
    <a-table :scroll="{x:2200,y:500}" bordered ref="table" size="middle" rowKey="id" :dataSource="dataSource"
    ...
    <!-- table区域-end -->
    <!--编辑-->
    <a-modal width="900px" :confirmLoading="confirmLoading" title="编辑流程" :visible="editObj.visible" @ok="editObjOk"
      ...
      <a-form :form="editForm" v-if="editObj.visible">
        ...
        <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="流程类型">
          <!-- <j-tree-dict placeholder="请选择流程类型" parentCode="B03"
                       :trigger-change="true" v-decorator="[ 'typeId', {initialValue:editObj.typeId, rules: [{ required: true, message: '不能为空' }] },]"
          >
          </j-tree-dict> -->
          <component :is="LcDict" :trigger-change="true" v-decorator="[ 'typeId', {initialValue:editObj.typeId, rules: [{ required: true, message: '不能为空' }] },]"
            placeholder="请选择流程类型" dictCode="handle_type"></component>
        </a-form-item>
    ...
    <!--节点设置-->
    <a-modal title="编辑流程节点" width="900px" :maskClosable="false" :confirmLoading="confirmLoading" :visible="showProcessNodeEdit"
      :footer="null" @cancel="closeNode">
      <a-row>
        <a-col :md="4" :sm="4" style="border-right: 1px solid #999">
          ...
          <a-steps direction="vertical" :current="current" size="small">
            <template v-for="item,i in nodeList">
              <a-step style="cursor: pointer;" :title="item.title" :description="item.description" :status="i==current?'process':'wait'"
        ...
        <a-col :md="20" :sm="20">
          ...
          <a-form :form="nodeForm" v-if="showProcessNodeEdit">
            ...
            <br />
            <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="联合办理" v-show="editNode.type==1">
              <a-radio :default-checked="editNode.unite == 0?false:true" :disabled="true">
                <template v-if="editNode.unite == 0">
                  不
                </template>需要
              </a-radio>
              <a-button type="primary" @click="uniteType">
                更改设置
              </a-button>
            </a-form-item>
            <a-divider>节点设置</a-divider>
            <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="审批人员" v-show="editNode.type==1">
              <a-checkbox-group @change="spryType" v-model="spryTypes">
                <!-- 0角色 1用户 2部门 3发起人 4发起人的部门负责人 5发起人的上级部门指定角色 6指定部门下的所有人员-->
                ...
                <a-checkbox value="5">
                  发起人的上级部门指定角色
                  <a-tooltip placement="topLeft" title="自动获取发起人所在部门的上级部门的指定角色">
                    <a-icon type="exclamation-circle" />
                  </a-tooltip>
                </a-checkbox>
                <a-checkbox value="6">
                  指定部门下的所有人员
                  <a-tooltip placement="topLeft" title="自动获取指定部门下的所有人员">
                    <a-icon type="exclamation-circle" />
                  </a-tooltip>
                </a-checkbox>
                <a-checkbox value="7">
                  各支线自主配置
                  <a-tooltip placement="topLeft" title="各支线自主配置,可子定义部门或者角色">
                    <a-icon type="exclamation-circle" />
                  </a-tooltip>
                </a-checkbox>
              </a-checkbox-group>
            </a-form-item>
            <!--            0角色 1用户 2部门 3发起人 4发起人的部门负责人 5 发起人的上级部门的指定角色 6指定部门下的所有人员-->
            ...
            <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="选择指定角色" v-if="spryTypes.indexOf('5')>-1">
              <j-select-role v-model="spry.appointRoleIds" />
            </a-form-item>
            <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="选择指定部门" v-if="spryTypes.indexOf('6')>-1">
              <j-select-depart v-model="spry.appointDepartIds" :multi="false"></j-select-depart>
            </a-form-item>

            <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="支线可指派角色" v-if="spryTypes.indexOf('7')>-1">
              <j-select-role v-model="spry.customizedRoleIds" />
            </a-form-item>

            <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="支线可指派部门" v-if="spryTypes.indexOf('7')>-1">
              <j-select-depart v-model="spry.customizedDepartIds" :multi="false"></j-select-depart>
              <a-checkbox v-model='spry.autoDepartFlag' name="type" @change="onChangeDepartFlag">
                自动指定操作人的部门
              </a-checkbox>
            </a-form-item>
            <!--btn-->
    ...
    <a-modal :title="uniteTitle" v-model="uniteVisible" :mask-closable="false" :width="500">

      <div v-if="uniteVisible">
        <a-form ref="form" :model="uniteForm" :label-width="85">
          <a-alert message="办理部门和办理角色 至少填写一个,不填写则改为不需要" type="info" show-icon />
          <a-form-item label="联合办理角色">
            <j-select-role v-model="uniteForm.uniteRoleIds" />
          </a-form-item>
          <a-form-item label="联合办理部门">
            <j-select-depart v-model="uniteForm.uniteDepartId" :multi="false"></j-select-depart>
          </a-form-item>
        
        </a-form>
      </div>
      <div slot="footer">
        <a-button type="text" @click="uniteCancel">取消</a-button>
        <a-button type="primary" :loading="submitLoading" @click="handelUniteSubmit">保存</a-button>
      </div>
    </a-modal>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          ...
          saveNodeUnite: "/activiti_process/saveNodeUnite",
        },
        spry: {
          //选中的用户
          userIds: '',
          ...
          chooseDepRole: false,
          chooseDepUser: false,
          chooseCustomized: false,
          autoDepartFlag:false,
        },
        lcModa: {
          title: '流程表单预览',
          visible: false,
    ...
    methods: {
      ...
      change_steps(node, index) {
        ...
        this.current = index;
        this.editNode = node;
        /* 0角色 1用户 2部门 3发起人 4发起人的部门负责人 5发起人上级部门的指定角色 6部门下的所有人员 7自定义配置*/
        ...
        if (roleIds.length > 0 && !node.chooseDepRole && !node.chooseCustomized) this.spryTypes.push('0');

        this.spry.chooseDepRole = node.chooseDepRole || false;
        if (this.spry.chooseDepRole) {
          this.spryTypes.push('5');
          var appointRoleIds = roleIds;
          this.spry.appointRoleIds = appointRoleIds.join(",")
        }
        ...
        if (departmentIds.length > 0 && !node.chooseDepUser && !node.chooseCustomized) this.spryTypes.push('2');

        this.spry.chooseDepUser = node.chooseDepUser || false;
        if (this.spry.chooseDepUser) {
          this.spryTypes.push('6');
          var appointDepartIds = departmentIds;
          this.spry.appointDepartIds = appointDepartIds.join(",");
        }

        this.spry.chooseCustomized = node.chooseCustomized || false;

        if (this.spry.chooseCustomized) {
          this.spryTypes.push('7');

          var customizedRoleIds = roleIds;
          this.spry.customizedRoleIds = customizedRoleIds.join(",")

          var customizedDepartIds = departmentIds;
          this.spry.customizedDepartIds = customizedDepartIds.join(",")
        }
        this.spry.autoDepartFlag = node.autoDepartFlag || false;
      ...
      spryType(types) {
        /* 0角色 1用户 2部门 3发起人 4发起人的部门负责人 5发起人上级部门的指定角色*/
        ...
        if (this.spryTypes.indexOf('5') == -1) this.spry.appointRoleIds = '';
        if (this.spryTypes.indexOf('6') == -1) this.spry.appointDepartIds = '';
        if (this.spryTypes.indexOf('7') == -1) this.spry.customizedRoleIds = '';
        if (this.spryTypes.indexOf('7') == -1) this.spry.customizedDepartIds = '';
        if (this.spryTypes.indexOf('7') == -1) this.spry.autoDepartFlag = false;
        //是否选中发起人
        this.spry.chooseSponsor = this.spryTypes.indexOf('3') > -1;
        //是否选中发起人的部门领导
        this.spry.chooseDepHeader = this.spryTypes.indexOf('4') > -1;
        //是否选中发起人的上级部门的相关角色
        this.spry.chooseDepRole = this.spryTypes.indexOf('5') > -1;
        //是否选中部门下所有人
        this.spry.chooseDepUser = this.spryTypes.indexOf('6') > -1;
        //是否选中自定义配置
        this.spry.chooseCustomized = this.spryTypes.indexOf('7') > -1;
      ...
      convertToModel(row) {
        ...
        this.$confirm({
          ...
          loading: true,
          onOk: () => {
            that.postFormAction(`${that.url.convertToModel}`, {
              id: row.id
            }).then(res => {
              if (res.success) {
                setTimeout(function() {
      ...
      remove(row) {
        ...
        _this.$confirm({
          ...
          loading: true,
          onOk: () => {
            _this.postFormAction(_this.url.delByIds,{ids: row.id}
            ).then(res => {
              if (res.success) {
                _this.$message.success("操作成功");
                _this.loadData();
      ...
      editStatus(status, row) {
        ...
        this.$confirm({
          ...
          onOk() {
            ...
            _this.postFormAction(_this.url.updateStatus, status,id).then(res => {
              if (res.success) {
                _this.$message.success("操作成功");
                _this.loadData();
              } else {
      ...
      uniteType() {
          ...
          this.uniteForm.defId = this.editNode.procDefId;
          this.uniteForm.nodeId = this.editNode.id.toString();
          if(this.editNode.uniteMap != null){
            this.uniteForm.uniteDepartId = this.editNode.uniteMap.u_departId;
            this.uniteForm.uniteRoleIds = this.editNode.uniteMap.u_roleIds;
            this.uniteForm.uniteUserIds = this.editNode.uniteMap.u_userIds;
          }
          console.log(this.uniteForm);
      },
      handelUniteSubmit() {
        console.log(this.uniteForm);
        var _this = this;
        var params = Object.assign({}, this.uniteForm);
        _this.postFormAction(_this.url.saveNodeUnite, params).then(res => {
          if (res.success) {
            console.log(res.result);
            var unite = res.result;
            _this.editNode.unite = unite.isUnite;
            _this.editNode.uniteMap = {
              u_departId:'',
              u_roleIds:'',
              u_userIds:''
            };
            _this.editNode.uniteMap.u_departId = unite.uniteDepartId+'';
            _this.editNode.uniteMap.u_roleIds = unite.uniteRoleIds+'';
            _this.editNode.uniteMap.u_userIds = unite.uniteUserIds+'';
            _this.uniteVisible = false;
          } else {
            _this.$message.error(res.message);
          }
        });
      ...
      uniteCancel() {
        this.uniteVisible = false;
        this.uniteForm.uniteDepartId = '';
        this.uniteForm.uniteRoleIds = '';
      },
      onChangeDepartFlag(e){
        this.spry.autoDepartFlag = e.target.checked;
      }
    }
  }
</script>
