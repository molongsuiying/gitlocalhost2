  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          ...
          getById:'/honour/file/queryHonourFile'
        }
      }
    },
    created() {
    ...
    methods: {
      ...
      pass(record) {
        ...
        this.visible = true
        console.log(this.model)
        getAction(this.url.getById,{id:this.model.tableId}).then((res)=>{
            if(res.success){
                console.log(res.result)
                // this.model = res.result
          if (res.result.id) {
          if (res.result.appendixIds != null) {
            this.getAppendix(this.model.appendixIds)
          }
          res.result.authorityType += ''
          if (res.result.authorityType == '99') {
            this.authorityFlag = true
          }
          res.result.achievementType += ''
          if (res.result.achievementType == '99') {
            this.achievementFlag = true
          }

          res.result.achievementLevel += ''
          if (res.result.achievementLevel == '99') {
            this.levelFlag = true
          }
          var persons = res.result.teamPersons
          if (persons !== null && persons.length !== 0) {
            this.teamPersons = persons.split(',')
          }
        }
