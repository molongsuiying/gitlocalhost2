    ...
    <div>
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <!-- <a v-has="'user:honourEdit'" @click="handleEdit(record)" v-show="record.status==0">编辑</a>
          <a-divider type="vertical" v-has="'user:honourEdit'" v-show="record.status==0"/>

          <a v-has="'user:honourSubmit'" @click="submit(record)" v-show="record.status==0 ">提交</a>
          <a-divider type="vertical" v-has="'user:honourSubmit'" v-show="record.status==0"/> -->
...
<script>
  ...
  import Vue from 'vue'
  import HonourArticleModal from './HonourArticleModal'
  ...
  export default {
    ...
    methods: {
      ...
      handleDetail(record) {
        record.honourClass = 3
        record.honourClass_dictText = '物图类'
