  ...
  export default {
    ...
    methods: {
      detail(record) {
        ...
        var params = {
          id: record.id,
          honourClass: record.honourClass
        }
        getAction(this.url.getDetail, params).then((res) => {
          if (res.success && res.result) {
            if (res.result.total > 0) {
              that.modal = res.result.records[0]
              that.modal.honourClass = record.honourClass
              that.modal.honourClass_dictText = record.honourClass_dictText
              that.modal.majorName = record.majorName
              this.getMyAuth(record.honourClass, record.appendixIds)
            }
          }
        })
      },
