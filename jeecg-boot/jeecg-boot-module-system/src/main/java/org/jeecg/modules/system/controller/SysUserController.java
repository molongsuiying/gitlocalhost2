...
public class SysUserController {
	...
	public Result<IPage<SysUser>> queryPageList(SysUser user,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
        ...
        if(userIds!=null && userIds.size()>0){
            ...
            Map<String,String> userRoleNames = sysUserService.getRoleNamesByUserIds(userIds);
            ...
            pageList.getRecords().forEach(item->{
                item.setRelTenantIds(userRoleNames.get(item.getId()));
            });
        }
    ...
    public Result<SysUser> updateMyInfo(@RequestBody JSONObject jsonObject) {
        Result<SysUser> result = new Result<SysUser>();
        try {
            LoginUser myInfo =  (LoginUser) SecurityUtils.getSubject().getPrincipal();
            SysUser user = JSON.parseObject(jsonObject.toJSONString(), SysUser.class);
            SysUser old = this.sysUserService.getById(myInfo.getId());
            old.setRealname(user.getRealname());
            old.setSex(user.getSex());
            old.setAvatar(user.getAvatar());
            old.setEmail(user.getEmail());
            old.setUpdateTime(new Date());
            this.sysUserService.updateById(old);
            sysBaseAPI.addLog("编辑用户，id： " +jsonObject.getString("id") ,CommonConstant.LOG_TYPE_2, 2);
...
//                json.put("avatar", item.getSysUser().getAvatar());
     ...
     * 部门用户列表
     */
    @RequestMapping(value = "/userListByRoleIdAndDepartId", method = RequestMethod.GET)
    public Result<IPage<SysUser>> userListByRoleIdAndDepartId(@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
                                                              @RequestParam(name="pageSize", defaultValue="10") Integer pageSize, HttpServletRequest req) {
        Result<IPage<SysUser>> result = new Result<IPage<SysUser>>();
        Page<SysUser> page = new Page<SysUser>(pageNo, pageSize);
        String depId = req.getParameter("subDepartId");
        String username = req.getParameter("username");

        String uniteFlag = req.getParameter("uniteFlag");
        ...
        if (StringUtils.isNotBlank(username)){
            if(username.contains("*")){
                ...
                userNames = Arrays.asList(username.split(","));
            }
        }
        ...
        String customizedRole = req.getParameter("customizedRole");
        ...
        if(StringUtils.isNotBlank(depId)){
            ...
            LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
            SysDepartModel depart = sysBaseAPI.getDepartByCode(user.getOrgCode());
            if(StringUtils.isNotBlank(uniteFlag) && Integer.parseInt(uniteFlag) == 1){
                //联合办理 获取所有同级及以下部门

                if(!StringUtils.isBlank(depart.getParentId())){
                    subDepids = sysDepartService.getSubDepIdsByDepId(depart.getParentId());
                    subDepids = subDepids.stream().filter(s -> !depart.getParentId().equals(s)).collect(Collectors.toList());
                }

            }else{

                subDepids = sysDepartService.getSubDepIdsByDepId(depart.getId());
            }

        }
        if(StringUtils.isNotBlank(customizedRole)){
            roleIds = Arrays.asList(customizedRole.split(","));
        }


        IPage<SysUser> pageList = sysUserService.getUserByRoleIdsAndDepartIds(page,roleIds,subDepids,userNames,username);
