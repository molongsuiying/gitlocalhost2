...
import org.jeecg.modules.system.vo.SysUserRoleNameVo;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
	...
	@Override
	public IPage<SysUser> getUserByRoleIdsAndDepartIds(Page<SysUser> page, List<String> roleIds, List<String> departIds,List<String> userNames, String username) {
		return userMapper.getUserByRoleIdsAndDepartIds(page,roleIds, departIds,userNames,username);
	}
	...
	public void addUserWithRole(SysUser user, String roles) {
		if(oConvertUtils.isNotEmpty(roles)) {
			...
			for (String roleId : arr) {
				...
				addAuthByNewUser(user.getId(),role.getRoleCode());
			}
		}

	}

	private void addAuthByNewUser(String userId,String roleCode){
		HonourAuth auth = new HonourAuth();
		auth.setUserId(userId);
		Integer yes = 1;
		Integer no = 0;
		auth.setAuthType(yes);
		auth.setEditAuth(yes);
		auth.setShowAuth(yes);
		auth.setDownloadAuth(yes);
		auth.setExportAuth(yes);
		auth.setExamineAuth(yes);
		if("achieve_entry_clerk".equals(roleCode)){
			auth.setExamineAuth(no);
		}else if("temporary_member".equals(roleCode)){
			auth.setAuthType(2);
			Calendar c = Calendar.getInstance();
			c.add(Calendar.DAY_OF_YEAR, c.get(Calendar.DAY_OF_YEAR) + 7);
			auth.setExamineAuth(no);
			auth.setExamineDeadline(c.getTime());
			auth.setExportAuth(no);
			auth.setExportDeadline(c.getTime());
		}else if("normal_member".equals(roleCode)){
			auth.setAuthType(2);
			auth.setExamineAuth(no);
			auth.setDownloadAuth(no);
			auth.setExamineAuth(no);
		}
...
//		info.setSysUserName(user.getRealname());
	...
	public Map<String, String> getRoleNamesByUserIds(List<String> userIds) {
		List<SysUserRoleNameVo> list = this.baseMapper.getRoleNamesByUserIds(userIds);
