...
import org.jeecg.modules.activiti.mapper.AppPermissionMapper;
...
//	private SysDepartRolePermissionMapper sysDepartRolePermissionMapper;
	...
	public List<TreeModel> queryListByParentId(String parentId) {
		return appPermissionMapper.queryListByParentId(parentId);
	}
	...
	public  void deletePermission(String id) throws JeecgBootException {
		AppPermission appPermission = this.getById(id);
		if(appPermission==null) {
			throw new JeecgBootException("未找到菜单信息");
		}
		String pid = appPermission.getParentId();
		if(oConvertUtils.isNotEmpty(pid)) {
			Integer count = this.count(new QueryWrapper<AppPermission>().lambda().eq(AppPermission::getParentId, pid));
			if(count==1) {
				//若父节点无其他子节点，则该父节点是叶子节点
				this.appPermissionMapper.setMenuLeaf(pid, 1);
			}
		}
...
//		sysDepartRolePermissionMapper.deleteByMap(map);
	...
	public void removeChildrenBy(String parentId) {
		...
		// 查出该主键下的所有子级
		List<AppPermission> permissionList = this.list(query);
...
//				//删除部门角色授权
//				sysDepartRolePermissionMapper.deleteByMap(map);
				num = this.count(new LambdaQueryWrapper<AppPermission>().eq(AppPermission::getParentId, id));
				// 如果有, 则递归
				if (num > 0) {
	...
	public void deletePermissionLogical(String id) throws JeecgBootException {
		AppPermission sysPermission = this.getById(id);
	...
	public void addPermission(AppPermission sysPermission) throws JeecgBootException {
		...
		if(oConvertUtils.isNotEmpty(pid)) {
			//设置父节点不为叶子节点
			this.appPermissionMapper.setMenuLeaf(pid, 0);
		}
	...
	public void editPermission(AppPermission sysPermission) throws JeecgBootException {
		AppPermission p = this.getById(sysPermission.getId());
		//TODO 该节点判断是否还有子节点
		if(p==null) {
			throw new JeecgBootException("未找到菜单信息");
		}else {
			...
			Integer count = this.count(new QueryWrapper<AppPermission>().lambda().eq(AppPermission::getParentId, sysPermission.getId()));
			if(count==0) {
				sysPermission.setLeaf(true);
			}
			//----------------------------------------------------------------------
			...
			if((oConvertUtils.isNotEmpty(pid) && !pid.equals(p.getParentId())) || oConvertUtils.isEmpty(pid)&&oConvertUtils.isNotEmpty(p.getParentId())) {
				...
				Integer cc = this.count(new QueryWrapper<AppPermission>().lambda().eq(AppPermission::getParentId, p.getParentId()));
				if(cc==0) {
					if(oConvertUtils.isNotEmpty(p.getParentId())) {
						this.appPermissionMapper.setMenuLeaf(p.getParentId(), 1);
					}
				}
	...
	public List<Role> queryAllSys() {
		return appPermissionMapper.queryAllSys();
	}
	//	@Override
