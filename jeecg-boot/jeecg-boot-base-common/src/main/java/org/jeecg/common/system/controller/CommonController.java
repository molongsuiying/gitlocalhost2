...
import org.jeecg.common.util.AsposeUtil;
...
public class CommonController {
	...
	public Result<?> uploadFile(HttpServletRequest request, HttpServletResponse response) {
		...
		jsonObject = this.uploadLocal2(file,bizPath);
	...
	public Result<String> wordToPdf(@Param("wordPath") String wordPath, HttpServletRequest request, HttpServletResponse response){
		String ctxPath = uploadpath;
		wordPath = ctxPath + File.separator + wordPath;
		Format format = new SimpleDateFormat("yyMMdd");
		String time = format.format(new Date());
		Result<String> result = new Result<>();
		try {
			String fileName = wordPath.substring(wordPath.lastIndexOf("/")+1, wordPath.lastIndexOf(".")) + ".pdf";
			String pdfPath =  "w2p" + File.separator + time +  File.separator + fileName;
			AsposeUtil.doc2pdf(wordPath,ctxPath + File.separator + pdfPath);

			result.setResult(pdfPath);
			result.setSuccess(true);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	 ...
	 * @return
	 */
	private JSONObject uploadLocal2(MultipartFile mf,String bizPath){
		...
		try {
			...
			if(orgName.contains(".")){
				//fileName = orgName.substring(0, orgName.lastIndexOf(".")) + "_" + System.currentTimeMillis() + orgName.substring(orgName.indexOf("."));
				fileName = orgName.substring(0, orgName.lastIndexOf("."))+"_"+oConvertUtils.randomGen(12)+orgName.substring(orgName.indexOf("."));
			}else{
				fileName = orgName+ "_" + System.currentTimeMillis();
			}
			String savePath = file.getPath() + File.separator + fileName;
			...
			String appendixId = sysBaseAPI.addAppendix(fileName,suffix,df.format(size),dbpath);

			jsonObject.put("path",dbpath);
			jsonObject.put("appendixId",appendixId);
	...
	public Result<List<String>> uploadMultipartFile(HttpServletRequest request, HttpServletResponse response) {
		Result<List<String>> result = new Result<>();
		String savePath = "";
		String bizPath = request.getParameter("biz");
		MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
		List<MultipartFile> fileList = multipartRequest.getFiles("files[]");// 获取上传文件对象;
		...
		for (int i = 0; i < fileList.size(); i++) {
			MultipartFile file = fileList.get(i);// 获取上传文件对象
			savePath = this.uploadLocal(file,bizPath);
			paths.add(savePath);
		}
		if(paths.size()>0){
	 ...
	 * @return
	 */
	@PostMapping(value = "/uploadMultipartFileByHonour")
	public Result<List<JSONObject>> uploadMultipartFileByHonour(HttpServletRequest request, HttpServletResponse response) {
